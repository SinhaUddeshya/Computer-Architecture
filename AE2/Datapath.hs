...
datapath ctlsigs memdat = (ma,md,cond,a,b,ir,pc,ad,ovfl,r,x,y,p,ready1,prod,rx,ry,s)
  where
...
-- Registers
      (a,b) = regfile n k (ctl_rf_ld ctlsigs) (mux1w (ctl_rf_increment ctlsigs) ir_d ir_sa) rf_sa rf_sb p -- If the rf increment signal is 0 then use ir_d as input, else if 1 use ir_sa
...
-- Internal processor signals
      ...
      p  = mux1w (ctl_rf_pc ctlsigs)                -- regfile data input
             ...
             (mux1w (ctl_rf_mul ctlsigs) pc prod)
...
-- Multiply
      ...
      (ready1,prod0,rx,ry,s) = multiply n (ctl_mul_st ctlsigs) x y
      prod = field prod0 16 16 
